...
/*
 * Motic (svs, tif) support, which is the same as Aperio but with Motic in the image description
 *
...
static bool motic_detect(const char *filename G_GNUC_UNUSED,
  ...
  if (!g_str_has_prefix(tagval, MOTIC_DESCRIPTION)) {
    g_set_error(err, OPENSLIDE_ERROR, OPENSLIDE_ERROR_FAILED,
                "Not a Motic slide");
    return false;
  }
...
static void add_properties(openslide_t *osr, char **props) {
  ...
  _openslide_duplicate_int_prop(osr, "motic.AppMag",
                                OPENSLIDE_PROPERTY_NAME_OBJECTIVE_POWER);
  _openslide_duplicate_double_prop(osr, "motic.MPP",
...
static bool motic_open(openslide_t *osr,
                        const char *filename,
