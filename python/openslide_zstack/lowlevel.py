""" low level operators to be used by the zstack image class
"""
from __future__ import division
from __future__ import print_function
from ctypes import (c_int32, c_uint32, c_char_p, c_int64, c_void_p, POINTER, byref, c_double, cdll)
import ctypes.util
...
if platform.system() == 'Windows':
    ...
    try:
        ...
        if _lib is None:
            raise ImportError("Couldn't locate OpenSlide dylib.  Is OpenSlide installed?")
...
try:
    ...
    print('Could not import _convert')
    ...
    def _load_image(buf, size):
        ...
        for y in range(0, h, rows_per_load):
            ...
            if sys.version[0] == '2':
                chunk = buffer(buf, 4 * y * w, 4 * rows * w) # pylint: disable=undefined-variable
            else:
...
class _OpenSlide(object):
    ...
    def invalidate(self):
        """ Invalidate the file so that we don't attempt to close it on shutdown """
        self._valid = False

    @classmethod
    def from_param(cls, obj):
        """ Method used to allow this class to be passed in an argtypes sequence """
        if obj.__class__ != cls:
            raise ValueError("Not an OpenSlide reference")
        if not obj._as_parameter_: # pylint: disable=W0212
            raise ValueError("Passing undefined slide object")
        if not obj._valid: # pylint: disable=W0212
...
class _utf8_p(object):
    ...
    if sys.version[0] == '2':
        _bytes_type = str
        _str_type = unicode # pylint: disable=undefined-variable
    else:
        _bytes_type = bytes
        _str_type = str
...
open_img = _func('openslide_open', c_void_p, [_utf8_p], _check_open)

close = _func('openslide_close', None, [_OpenSlide], _check_close)

get_level_count = _func('osz_get_level_count', c_int32, [_OpenSlide, c_int32])

get_zlevel_count = _func('osz_get_zlevel_count', c_int32, [_OpenSlide])


_get_zlevel_offset = _func('osz_get_zlevel_offset', None, [_OpenSlide, c_int32, POINTER(c_double)])
def get_zlevel_offset(slide, zlevel):
    """ Wrapper around osz_get_zlevel_offset
    """
    offset = c_double()
    _get_zlevel_offset(slide, zlevel, byref(offset))
    return offset.value

_get_level_dimensions = _func('osz_get_level_dimensions', None, [_OpenSlide, c_int32, c_int32, POINTER(c_int64), POINTER(c_int64)])
def get_level_dimensions(slide, zlevel, level):
    """ Wrapper around osz_get_level_dimension
    """
    w, h = c_int64(), c_int64()
    _get_level_dimensions(slide, zlevel, level, byref(w), byref(h))
    return w.value, h.value

get_level_downsample = _func('osz_get_level_downsample', c_double, [_OpenSlide, c_int32, c_int32])

get_best_level_for_downsample = _func('osz_get_best_level_for_downsample', c_int32, [_OpenSlide, c_int32, c_double])

_read_region = _func('osz_read_region', None, [_OpenSlide, POINTER(c_uint32), c_int32, c_int64, c_int64, c_int32, c_int64, c_int64])
def read_region(slide, zlevel, x, y, level, w, h):
    """ Wrapper around osz_read_region
    """
    if w < 0 or h < 0:
        # OpenSlide would catch this, but not before we tried to allocate
        # a negative-size buffer
        raise OpenSlideError( "negative width (%d) or negative height (%d) not allowed" % ( w, h))
    if w == 0 or h == 0:
        # PIL.Image.frombuffer() would raise an exception
        return PIL.Image.new('RGBA', (w, h))
    buf = (w * h * c_uint32)()
    _read_region(slide, buf, zlevel, x, y, level, w, h)
    return _load_image(buf, (w, h))

get_error = _func('openslide_get_error', c_char_p, [_OpenSlide], _check_string)

get_property_names = _func('openslide_get_property_names', POINTER(c_char_p), [_OpenSlide], _check_name_list)

get_property_value = _func('openslide_get_property_value', c_char_p, [_OpenSlide, _utf8_p])

get_associated_image_names = _func('openslide_get_associated_image_names', POINTER(c_char_p), [_OpenSlide], _check_name_list)

_get_associated_image_dimensions = _func('openslide_get_associated_image_dimensions', None, [_OpenSlide, _utf8_p, POINTER(c_int64), POINTER(c_int64)])
def get_associated_image_dimensions(slide, name):
    """ Wrapper around openslide_get_associated_image_dimensions
    """
    w, h = c_int64(), c_int64()
    _get_associated_image_dimensions(slide, name, byref(w), byref(h))
    return w.value, h.value

_read_associated_image = _func('openslide_read_associated_image', None, [_OpenSlide, _utf8_p, POINTER(c_uint32)])
def read_associated_image(slide, name):
    """ Wrapper around openslide_read_associated_image
    """
...
set_tiff_message_verbosity = _func('set_tiff_message_verbosity', None, [c_int32], None)
